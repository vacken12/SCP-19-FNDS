...
/datum/species/shapeshifter/promethean
	...
	valid_transform_species = list(SPECIES_HUMAN, SPECIES_UNATHI, SPECIES_SKRELL, SPECIES_MANTID_ALATE, SPECIES_MONKEY)
	monochromatic = 1
...
/datum/species/shapeshifter/promethean/get_additional_examine_text(mob/living/carbon/human/H)
	...
	switch(stored_shock_by_ref["\ref[H]"])
		if(1 to 10)
			return SPAN_NOTICE("<i>[H.p_they(TRUE)] [H.p_are()] flickering gently with a little electrical activity.</i>")
		if(11 to 20)
			return SPAN_NOTICE("<i>[H.p_they(TRUE)] [H.p_are()] glowing gently with moderate levels of electrical activity.</i>")
		if(21 to 35)
			return SPAN_WARNING("[H.p_they(TRUE)] [H.p_are()] glowing brightly with high levels of electrical activity.")
		if(35 to INFINITY)
			return SPAN_DANGER("[H.p_they(TRUE)] [H.p_are()] radiating massive levels of electrical activity!")
